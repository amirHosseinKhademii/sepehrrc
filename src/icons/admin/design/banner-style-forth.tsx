import { FC } from 'react';

export const ICBannerStyleForth: FC<ISvg> = ({
  className,
  onClick,
  height,
  width,
  fill,
}) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={width ? width : 234}
      height={height ? height : 84}
      viewBox="0 0 234 84"
      className={className}
      onClick={onClick}
    >
      <g id="Banner-Style-4" transform="translate(-894 -602)">
        <g
          id="Group_1428"
          data-name="Group 1428"
          transform="translate(560 312)"
        >
          <g
            id="Group_1299"
            data-name="Group 1299"
            transform="translate(334 290)"
          >
            <rect
              id="Rectangle_12122"
              data-name="Rectangle 12122"
              width="70"
              height="36"
              rx="2"
              fill="#1578e6"
            />
          </g>
          <path
            id="download"
            d="M23.5,7.5H8.5a1,1,0,0,0-1,1v15a1,1,0,0,0,1,1h15a1,1,0,0,0,1-1V8.5A1,1,0,0,0,23.5,7.5Zm-3.374,3.488A1.311,1.311,0,1,1,18.815,12.3,1.313,1.313,0,0,1,20.126,10.988Zm1.96,9.974a.1.1,0,0,1-.086.05H10a.1.1,0,0,1-.087-.149l3.358-5.9a.1.1,0,0,1,.086-.051.117.117,0,0,1,.087.048l1.2,1.98L17.3,12.663a.1.1,0,0,1,.172,0l4.618,8.2A.1.1,0,0,1,22.086,20.962Z"
            transform="translate(353.133 292.5)"
            fill="#36a7ff"
          />
        </g>
        <g
          id="Group_1299-2"
          data-name="Group 1299"
          transform="translate(976 602)"
        >
          <rect
            id="Rectangle_12122-2"
            data-name="Rectangle 12122"
            width="70"
            height="84"
            rx="2"
            fill="#1578e6"
          />
        </g>
        <path
          id="download-2"
          data-name="download"
          d="M23.5,7.5H8.5a1,1,0,0,0-1,1v15a1,1,0,0,0,1,1h15a1,1,0,0,0,1-1V8.5A1,1,0,0,0,23.5,7.5Zm-3.374,3.488A1.311,1.311,0,1,1,18.815,12.3,1.313,1.313,0,0,1,20.126,10.988Zm1.96,9.974a.1.1,0,0,1-.086.05H10a.1.1,0,0,1-.087-.149l3.358-5.9a.1.1,0,0,1,.086-.051.117.117,0,0,1,.087.048l1.2,1.98L17.3,12.663a.1.1,0,0,1,.172,0l4.618,8.2A.1.1,0,0,1,22.086,20.962Z"
          transform="translate(995.5 628.5)"
          fill="#36a7ff"
        />
        <g
          id="Group_1430"
          data-name="Group 1430"
          transform="translate(724 312)"
        >
          <g
            id="Group_1299-3"
            data-name="Group 1299"
            transform="translate(334 290)"
          >
            <rect
              id="Rectangle_12122-3"
              data-name="Rectangle 12122"
              width="70"
              height="36"
              rx="2"
              fill="#1578e6"
            />
          </g>
          <path
            id="download-3"
            data-name="download"
            d="M23.5,7.5H8.5a1,1,0,0,0-1,1v15a1,1,0,0,0,1,1h15a1,1,0,0,0,1-1V8.5A1,1,0,0,0,23.5,7.5Zm-3.374,3.488A1.311,1.311,0,1,1,18.815,12.3,1.313,1.313,0,0,1,20.126,10.988Zm1.96,9.974a.1.1,0,0,1-.086.05H10a.1.1,0,0,1-.087-.149l3.358-5.9a.1.1,0,0,1,.086-.051.117.117,0,0,1,.087.048l1.2,1.98L17.3,12.663a.1.1,0,0,1,.172,0l4.618,8.2A.1.1,0,0,1,22.086,20.962Z"
            transform="translate(353 292.5)"
            fill="#36a7ff"
          />
        </g>
        <g
          id="Group_1431"
          data-name="Group 1431"
          transform="translate(560 360)"
        >
          <g
            id="Group_1299-4"
            data-name="Group 1299"
            transform="translate(334 290)"
          >
            <rect
              id="Rectangle_12122-4"
              data-name="Rectangle 12122"
              width="70"
              height="36"
              rx="2"
              fill="#1578e6"
            />
          </g>
          <path
            id="download-4"
            data-name="download"
            d="M23.5,7.5H8.5a1,1,0,0,0-1,1v15a1,1,0,0,0,1,1h15a1,1,0,0,0,1-1V8.5A1,1,0,0,0,23.5,7.5Zm-3.374,3.488A1.311,1.311,0,1,1,18.815,12.3,1.313,1.313,0,0,1,20.126,10.988Zm1.96,9.974a.1.1,0,0,1-.086.05H10a.1.1,0,0,1-.087-.149l3.358-5.9a.1.1,0,0,1,.086-.051.117.117,0,0,1,.087.048l1.2,1.98L17.3,12.663a.1.1,0,0,1,.172,0l4.618,8.2A.1.1,0,0,1,22.086,20.962Z"
            transform="translate(353.133 292.5)"
            fill="#36a7ff"
          />
        </g>
        <g
          id="Group_1433"
          data-name="Group 1433"
          transform="translate(724 360)"
        >
          <g
            id="Group_1299-5"
            data-name="Group 1299"
            transform="translate(334 290)"
          >
            <rect
              id="Rectangle_12122-5"
              data-name="Rectangle 12122"
              width="70"
              height="36"
              rx="2"
              fill="#1578e6"
            />
          </g>
          <path
            id="download-5"
            data-name="download"
            d="M23.5,7.5H8.5a1,1,0,0,0-1,1v15a1,1,0,0,0,1,1h15a1,1,0,0,0,1-1V8.5A1,1,0,0,0,23.5,7.5Zm-3.374,3.488A1.311,1.311,0,1,1,18.815,12.3,1.313,1.313,0,0,1,20.126,10.988Zm1.96,9.974a.1.1,0,0,1-.086.05H10a.1.1,0,0,1-.087-.149l3.358-5.9a.1.1,0,0,1,.086-.051.117.117,0,0,1,.087.048l1.2,1.98L17.3,12.663a.1.1,0,0,1,.172,0l4.618,8.2A.1.1,0,0,1,22.086,20.962Z"
            transform="translate(353 292.5)"
            fill="#36a7ff"
          />
        </g>
      </g>
    </svg>
  );
};
